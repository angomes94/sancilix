      ...
      >
        {imagePairs.map((pair, index) => (
          <SwiperSlide key={index}>
            <div className="w-full h-full flex flex-col justify-center items-center md:px-20">
                <div className="w-full h-1/2 p-2 md:p-10">
                  <h1 className="font-semibold text-xl text-center pb-5 ">{translations.slideBefore}</h1>
                  <img src={urlFor(pair.before).fit('clip').width(1200).height(600).url()} className="w-full h-full object-contain" />
                </div>
                <div className="w-full h-1/2 p-2 md:p-10">
                  <h1 className="font-semibold text-xl text-center pb-5 ">{translations.slideAfter}</h1>
                  <img src={urlFor(pair.after).fit('clip').width(1200).height(600).url()} className="w-full h-full object-contain" />
              </div>
            </div>
          </SwiperSlide>
        ))}
