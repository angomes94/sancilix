...
export default function Projects({ projects, renovations }) {
  ...
  return (
    <div id='projects' className='flex flex-col w-full'>
      ...
      {isProjectsTab ? 
      <div className=' grid grid-cols-1 gap-12 m-10 content-center justify-center items-center sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 '>
        {projects && projects.map((p) => (
          <Link key={p._id} href={`/project/${p._id}`} className=' duration-300 hover:scale-105 hover:opacity-100 opacity-75 cursor-pointer'>
        ...
        {renovations && renovations.map((p) => (
          <Link key={p._id} href={`/renovation/${p._id}`} className=' duration-300 hover:scale-105 hover:opacity-100 opacity-75 cursor-pointer'>
