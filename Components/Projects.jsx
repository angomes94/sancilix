    ...
    <div id='projects' className='flex flex-col w-full'>
      <div className=' flex flex-row w-full justify-center items-center bg-blue-950 p-10'>
        <div className=' flex flex-row w-3/4 text-xs md:text-lg justify-center items-center'>
          <button className={`${isProjectsTab ? "bg-white text-blue-950" : "bg-blue-600  text-white"} w-32 md:w-60 rounded-l-3xl p-2 md:p-5 `} onClick={() => setIsProjectsTab(true)}>{translations.buttonProjects}</button>
          <button className={`${isProjectsTab ? "bg-blue-600  text-white" : "bg-white text-blue-950"} w-32 md:w-60 rounded-r-3xl p-2 md:p-5`} onClick={() => setIsProjectsTab(false)}>{translations.buttonRenovations}</button>
        </div>
      </div>
      {isProjectsTab ? 
